...
// Packed Q15 complex: high16 = re, low16 = im. Matches the cmulCi data layout.
static uint32_t samples_cplx[FFT_N] __attribute__((aligned(4)));
...
int main(void) {
  ...
  while (1) {
    ...
    for (int i = 0; i < FFT_N; i++) {
      // im is implicit zero — packed layout makes the FFT's first load free
      samples_cplx[i] = (uint32_t)(uint16_t)i2s_read_blocking(0) << 16;
    }
    profile_stop(&pCap);

    profile_start();
    apply_hann(samples_cplx);
    profile_stop(&pWin);

    profile_start();
    fft_q15(samples_cplx);
    profile_stop(&pFft);

    profile_start();
    for (int k = 0; k < FFT_N / 2; k++) {
      mag_bins[k] = mag_approx_packed(samples_cplx[k]);
    }
    profile_stop(&pMag);
    ...
    if (status & I2S_STATUS_OVERRUN) {
      overrun_count++;
      i2s_ack_overrun();   // clear the flag but KEEP the FIFO contents
    }
    ...
    if ((frame & 0x1Fu) == 0u) {
      ...
      // One short line so the UART block stays under the audio period (no print-induced overruns)
      printf("f%d %dms", frame, total_ms);
      profile_print_labeled("cap", &aCap);
      profile_print_labeled("han", &aWin);
      profile_print_labeled("fft", &aFft);
      profile_print_labeled("mag", &aMag);
      profile_print_labeled("ren", &aRen);
      printf(" ov%d/32\r\n", overrun_count);
