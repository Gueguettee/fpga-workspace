module i2sReceiver #( parameter [31:0] baseAddress  = 32'h50000060,
  ...
  reg  enable_r;
  reg  irqEnable_r;
  // CTRL[1] = clear FIFO + clear overrun  (full reset)
  // CTRL[3] = clear overrun only (FIFO contents preserved — used to ack a sticky flag)
  wire ctrlWrite_s         = writeEnable_s && (regSel_s == 2'd2);
  wire clearPulse_s        = ctrlWrite_s && dataIn_r[1];
  wire clearOverrunPulse_s = ctrlWrite_s && dataIn_r[3];
  ...
  // ===================================================================
  // 1024-deep × 32-bit FIFO  (low 16 = sample, high 16 = counter)
  // 1024 entries ≈ 64 ms of audio at 16.1 kHz Fs — one full FFT batch.
  // Synchronous-read pattern so yosys infers DP16KD block RAM
  // (2 × DP16KD for 1024 × 32) instead of slice-LUT distributed RAM.
  // ===================================================================
  localparam integer FIFO_DEPTH    = 1024;
  localparam integer FIFO_ADDR_W   = 10;     // log2(FIFO_DEPTH)
  localparam integer FIFO_COUNT_W  = 11;     // 0..FIFO_DEPTH inclusive
  localparam integer FIFO_HALF     = FIFO_DEPTH / 2;

  reg  [31:0]              fifoMem_r [0:FIFO_DEPTH-1];
  reg  [FIFO_ADDR_W-1:0]   wrPtr_r, rdPtr_r;
  reg  [FIFO_COUNT_W-1:0]  fifoCount_r;
  reg  [31:0]              fifoHead_r;       // registered read (BRAM output)

  wire fifoEmpty_s = (fifoCount_r == 0);
  wire fifoFull_s  = (fifoCount_r == FIFO_DEPTH);
  ...
  always @(posedge clock) begin
    if (reset == 1'b1 || clearPulse_s) begin
      wrPtr_r     <= {FIFO_ADDR_W{1'b0}};
      rdPtr_r     <= {FIFO_ADDR_W{1'b0}};
      fifoCount_r <= {FIFO_COUNT_W{1'b0}};
    end else begin
      if (fifoWe_s) begin
        fifoMem_r[wrPtr_r] <= sampleWord_s;
        wrPtr_r <= wrPtr_r + 1'b1;
      end
      if (fifoRe_s)
        rdPtr_r <= rdPtr_r + 1'b1;
      case ({fifoWe_s, fifoRe_s})
        2'b10  : fifoCount_r <= fifoCount_r + 1'b1;
        2'b01  : fifoCount_r <= fifoCount_r - 1'b1;
        default: ;
      endcase
    end
  end

  // Free-running synchronous read — gives BRAM a stable address every cycle.
  // The 1-cycle latency hides inside the bus protocol's IDLE→WAIT transition.
  always @(posedge clock) fifoHead_r <= fifoMem_r[rdPtr_r];

  // overrun: sticky bit set when a new sample is pushed but FIFO is full
  // Cleared by either CTRL[1] (full clear) or CTRL[3] (overrun-only ack).
  reg overrun_r;
  wire overrunNow_s = pushSample_s && fifoFull_s;
  always @(posedge clock) begin
    if (reset == 1'b1 || clearPulse_s || clearOverrunPulse_s)
      overrun_r <= 1'b0;
  ...
  // Read mux + IRQ
  // ===================================================================
  // STATUS bit layout (32 bits, MSB→LSB):
  //   [31:13] = 0
  //   [12:2]  = FIFO occupancy (11 bits, range 0..1024)
  //   [1]     = overrun (sticky)
  //   [0]     = !empty  (sample available)
  reg [31:0] readMux_r;
  always @* begin
    case (regSel_s)
      2'd0    : readMux_r <= {19'd0, fifoCount_r, overrun_r, ~fifoEmpty_s};
      2'd1    : readMux_r <= fifoEmpty_s ? 32'd0 : fifoHead_r;
      default : readMux_r <= 32'd0;
    endcase
  end
  assign readValue_s = readMux_r;

  // IRQ asserts at half-full or more, gated by CTRL[2]
  assign irq = irqEnable_r && (fifoCount_r >= FIFO_HALF[FIFO_COUNT_W-1:0]);
