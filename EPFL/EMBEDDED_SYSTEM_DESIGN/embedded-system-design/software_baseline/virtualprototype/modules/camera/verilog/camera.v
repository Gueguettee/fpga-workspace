module camera #(parameter [7:0] customInstructionId = 8'd0,
  ...
  // Pack 4 grayscale bytes per 32-bit word → 8 pclk cycles between writes.
  ...
  // Convert each of the 2 RGB565 pixels in s_pixelWord to 8-bit grayscale.
  wire [7:0] grayPixelA_s, grayPixelB_s;
  rgb565Grayscale grayConvA ( .rgb565(s_pixelWord[15:0]),
                              .grayscale(grayPixelA_s) );
  rgb565Grayscale grayConvB ( .rgb565(s_pixelWord[31:16]),
                              .grayscale(grayPixelB_s) );

  // Latch the first pixel-pair's gray bytes; combine with the second pair to form a 4-pixel word.
  reg [15:0] grayLowHalf_r;
  always @(posedge pclk)
    grayLowHalf_r <= (s_pixelCountReg[2:0] == 3'b011 && hsync == 1'b1) ?
                     {grayPixelB_s, grayPixelA_s} : grayLowHalf_r;
  wire [31:0] grayPixelWord_s = {grayPixelB_s, grayPixelA_s, grayLowHalf_r};
  ...
  always @(posedge clock)
    begin
      ...
      // 4 pixels per 32-bit word now (vs 2 before) → halve the words-per-line.
