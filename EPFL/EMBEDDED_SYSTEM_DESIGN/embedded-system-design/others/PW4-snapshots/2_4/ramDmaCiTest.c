...
static volatile uint32_t dst_buf[256];

static uint32_t check_dst(uint32_t count, uint32_t pattern_base) {
  // CPU <-> bus crosses a byte-swap (same as tests 6-8), so undo it on readback.
  uint32_t errs = 0;
  for (uint32_t i = 0; i < count; i++) {
    uint32_t got = swap_u32(dst_buf[i]);
    uint32_t exp = pattern_base | i;
    if (got != exp) {
      if (!errs) printf("  first mismatch @%u got 0x%08X exp 0x%08X\n", i, got, exp);
      errs++;
...
static void test_dma_write_single_burst(void) {
  printf("[9] CI->bus DMA: blockSize=64, burstSize=255 (one burst of 64 words)\n");
  for (uint32_t i = 0; i < 64; i++) mem_write(300 + i, 0xABCD0000u | i);
  for (uint32_t i = 0; i < 64; i++) dst_buf[i] = 0xDEAD0000u | i;

  dma_set_base((uint32_t)&dst_buf[0]);
  dma_set_mem(300);
  dma_set_block(64);
  dma_set_burst(255);
  dma_ctrl(2);

  if (!wait_dma_done("write single-burst")) return;
  uint32_t s = dma_status();
  if (s & 2u) { printf("  FAIL bus error: status=0x%08X\n", s); fails++; return; }

  uint32_t errs = check_dst(64, 0xABCD0000u);
  if (errs) { printf("  FAIL %u mismatches\n", errs); fails++; }
  else      { printf("  OK\n"); }
}

static void test_dma_write_multi_burst(void) {
  printf("[10] CI->bus DMA: blockSize=16, burstSize=3 (4 bursts of 4 words)\n");
  for (uint32_t i = 0; i < 16; i++) mem_write(400 + i, 0x5EED0000u | i);
  for (uint32_t i = 0; i < 16; i++) dst_buf[i] = 0xBEEF0000u | i;
  ...
  if (!wait_dma_done("write multi-burst")) return;
  uint32_t s = dma_status();
  if (s & 2u) { printf("  FAIL bus error: status=0x%08X\n", s); fails++; return; }

  uint32_t errs = check_dst(16, 0x5EED0000u);
  if (errs) { printf("  FAIL %u mismatches\n", errs); fails++; }
  else      { printf("  OK\n"); }
}

static void test_dma_write_partial_last_burst(void) {
  printf("[11] CI->bus DMA: blockSize=10, burstSize=3 (2 full bursts + 1 partial of 2)\n");
  for (uint32_t i = 0; i < 10; i++) mem_write(450 + i, 0xFACE0000u | i);
  for (uint32_t i = 0; i < 10; i++) dst_buf[i] = 0xBABE0000u | i;
  ...
  if (!wait_dma_done("write partial-last")) return;
  uint32_t s = dma_status();
  if (s & 2u) { printf("  FAIL bus error: status=0x%08X\n", s); fails++; return; }

  uint32_t errs = check_dst(10, 0xFACE0000u);
  if (errs) { printf("  FAIL %u mismatches\n", errs); fails++; }
  else      { printf("  OK\n"); }
}

static void test_write_ctrl_3_rejected(void) {
  printf("[12] control=3 must still not start a write DMA\n");
  for (uint32_t i = 0; i < 4; i++) dst_buf[i] = 0x77770000u | i;
  dma_set_base((uint32_t)&dst_buf[0]);
  dma_set_mem(0);
  dma_set_block(4);
  dma_set_burst(3);
  ...
  for (uint32_t i = 0; i < 4; i++) {
    if (dst_buf[i] != (0x77770000u | i)) {
      if (!errs) printf("  first corruption @%u got 0x%08X\n", i, dst_buf[i]);
      errs++;
    }
  }
  if (errs) { printf("  FAIL %u dst_buf words changed\n", errs); fails++; }
  else      { printf("  OK (status=0x%08X, dst_buf intact)\n", s); }
}
...
int main (void) {
  ...
  test_dma_write_single_burst();
  test_dma_write_multi_burst();
  test_dma_write_partial_last_burst();
  test_write_ctrl_3_rejected();
