...
analyze -format sverilog mac_unit.sv
...
save_block -as ${DESIGN_NAME}/project1_read_rtl
...
report_area > reports/area_initial_map_report_p1.log
report_timing -significant_digits 10 > reports/timing_initial_map_report_p1.log

compile_fusion -from initial_map -to logic_opto
report_area > reports/area_logic_opto_report_p1.log
report_timing -significant_digits 10 > reports/timing_logic_opto_report_p1.log

#final optimization requires physical information, thus interconnects models are needed
set_app_options -name time.delay_calculation_style -value auto

compile_fusion -to final_opto

report_timing -significant_digits 10 > reports/timing_final_opto_report_interconnect_p1.log

#however, for the final reports, we want to use zero_interconnect to be consistent with previous runs
set_app_options -name time.delay_calculation_style -value zero_interconnect

report_area > reports/area_final_opto_report_p1.log
report_timing -significant_digits 10 > reports/timing_final_opto_report_p1.log

report_resources > reports/resource_final_opto_report_p1.log
report_utilization > reports/utilization_final_opto_report_p1.log

save_block -as ${DESIGN_NAME}/project1_final_opto
