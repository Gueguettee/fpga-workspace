...
void bit_reverse(FXP_TYPE *X_real, FXP_TYPE *X_imag, INT_NFFT_TYPE nfft, INT_NSTAGE_TYPE num_stages, FXP_TYPE *OUT_real, FXP_TYPE *OUT_imag) {
    for (INT_NFFT_TYPE i = 0; i < MAX_NFFT; i++) {
...
void fft_stage(INT_NSTAGE_TYPE stage, FXP_TYPE *X_real, FXP_TYPE *X_imag, INT_NFFT_TYPE nfft, FXP_TYPE *Out_real, FXP_TYPE *Out_imag) {
    ...
    for (INT_NFFT_TYPE j = 0; j < 1<<(MAX_NSTAGES-1); j++) {
    #pragma HLS LOOP_TRIPCOUNT min=(1<<(MAX_NSTAGES-1)) max=(1<<(MAX_NSTAGES-1))
        ...
        INT_NFFT_TYPE twiddle_idx = j << (MAX_NSTAGES-stage);
        ...
        for (INT_NFFT_TYPE i = j; i < MAX_NFFT; i += DFTpts) {
        #pragma HLS LOOP_TRIPCOUNT min=MAX_NFFT max=MAX_NFFT
...
void ReadInputs(INT_NSTAGE_TYPE log2_nfft, INT_NWORKERS_TYPE numHW,
                FXP_TYPE *In_real,
                FXP_TYPE *In_imag,
...
void WriteOutputs(INT_NSTAGE_TYPE numHW, INT_NSTAGE_TYPE log2_nfft, FXP_TYPE *Out_real, FXP_TYPE *Out_imag, hls::stream<TDataOut> streams[NUM_WORKERS])
{
    #pragma HLS inline off
