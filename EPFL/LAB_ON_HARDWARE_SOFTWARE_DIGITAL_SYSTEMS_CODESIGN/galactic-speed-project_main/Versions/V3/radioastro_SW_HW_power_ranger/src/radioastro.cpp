...
#if !USE_KERNEL
const uint32_t FFT_HW_ADDR = 0x40000000; // From Vivado's address editor
#else
const char * DRIVER_NAME = "/dev/fft";
#endif
...
///////////////////////////////////////////////////////////////////////////////
bool InitDevice(CXADCProxy & xadc, CFFTProxy & fft, bool log=false)
{
    if (log) {
    ...
    printf("Device at physical address 0x%08X successfully mapped into the application virtual address space\n\n", 
            FFT_HW_ADDR);
#else
    if ( fft.Open(DRIVER_NAME) != CAccelDriver::OK ) {
...
int main(int argc, char* argv[]) {
    ...
    CFFTProxy fftProxy(false);
    if (!InitDevice(powerRanger, fftProxy, false))
      return -1;
    ...
    if (config.log2_nfft > 16) {
        printf("\n\n###### Error, the hardware is not compatible with the last 3 solutions (solutions 7-8-9) !!!\n");
    }
    ...
    welch_psd(signal, Nseg, sample_rate, (int)signal_param.fc, freqs, config.log2_nfft, psd, times, &powerRanger, energies, &fftProxy);
    ...
    double cpuUtilization = (double)CalcTimeDiff(endCPUTime, startCPUTime) / (double)(times.timeTotal);
