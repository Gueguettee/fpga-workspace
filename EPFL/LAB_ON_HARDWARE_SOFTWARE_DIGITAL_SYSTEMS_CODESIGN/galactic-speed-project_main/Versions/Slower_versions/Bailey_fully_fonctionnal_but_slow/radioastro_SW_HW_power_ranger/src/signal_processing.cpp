...
void welch_psd(std::complex<float>* samples, int Nseg, float Fs, float Fc, float* freqs, int log2_nfft, float* spectrum, TTimes & times, CXADCProxy *powerRanger, TEnergies & energies, CFFTProxy *fftProxy){
    ...
    for (int i = 0; i < Nseg-3; i+=1){ // Slide 1 block
        ...
        */
        ...
        timeBailey += bailey_fft(input_real_fxp, input_imag_fxp, log2_nfft, outputHW_real_fxp, outputHW_imag_fxp, fftProxy);
                ...
                FXP_TYPE *Out_real, FXP_TYPE *Out_imag, CFFTProxy *fftProxy)
{
    ...
    FXP_TYPE *temp_real = (FXP_TYPE*)fftProxy->AllocDMACompatible(N * sizeof(FXP_TYPE));
    FXP_TYPE *temp_imag = (FXP_TYPE*)fftProxy->AllocDMACompatible(N * sizeof(FXP_TYPE));
    FXP_TYPE *temp_real2 = (FXP_TYPE*)fftProxy->AllocDMACompatible(N * sizeof(FXP_TYPE));
    FXP_TYPE *temp_imag2 = (FXP_TYPE*)fftProxy->AllocDMACompatible(N * sizeof(FXP_TYPE)); // FIXME why a second temp ??? -> to be sure that the input and output have different addresses, maybe it could cause an issue if the address in the same
    ...
    for (int col = 0; col < s; ++col)
    {
        for (int row = 0; row < r; ++row)
        {
            ...
            // printf("in real %.4f, in imag %.4f \n", Fxp2Float(temp_real[row]), Fxp2Float(temp_imag[row]));
        }

        clock_gettime(CLOCK_MONOTONIC_RAW, &start_segment);
        fftProxy->FFT_HW(temp_real, temp_imag, log2_r, temp_real2, temp_imag2);
        clock_gettime(CLOCK_MONOTONIC_RAW, &end_segment);
        time += CalcTimeDiff(end_segment, start_segment);

        for (int row = 0; row < r; ++row)
        {
            // printf("out real %.4f, out imag %.4f \n", Fxp2Float(temp_real2[row]), Fxp2Float(temp_imag2[row]));
            stage_real[IDX(row, col, s)] = temp_real2[row];
            stage_imag[IDX(row, col, s)] = temp_imag2[row];
        }
    }
    ...
    for (int row = 0; row < r; ++row)
    {
        for (int col = 0; col < s; ++col)
        {
            ...
            FXP_TYPE wr = Float2Fxp((float)cos(angle));
            FXP_TYPE wi = Float2Fxp((float)sin(angle));
            
            FXP_TYPE a_real = stage_real[IDX(row, col, s)];
            FXP_TYPE a_imag = stage_imag[IDX(row, col, s)];

            FXP_TYPE tw_real = FXP_Mult(a_real, wr) - FXP_Mult(a_imag, wi);
            FXP_TYPE tw_imag = FXP_Mult(a_real, wi) + FXP_Mult(a_imag, wr);
    ...
    for (int row = 0; row < r; ++row)
    {
        ...
        clock_gettime(CLOCK_MONOTONIC_RAW, &start_segment);
        fftProxy->FFT_HW(temp_real, temp_imag, log2_s, temp_real2, temp_imag2);
        clock_gettime(CLOCK_MONOTONIC_RAW, &end_segment);
        time += CalcTimeDiff(end_segment, start_segment);

        for (int col = 0; col < s; ++col)
        {
            stage_real[IDX(row, col, s)] = temp_real2[col];
            stage_imag[IDX(row, col, s)] = temp_imag2[col];
        }
    }
    ...
    fftProxy->FreeDMACompatible(temp_real2);
    fftProxy->FreeDMACompatible(temp_imag2);
