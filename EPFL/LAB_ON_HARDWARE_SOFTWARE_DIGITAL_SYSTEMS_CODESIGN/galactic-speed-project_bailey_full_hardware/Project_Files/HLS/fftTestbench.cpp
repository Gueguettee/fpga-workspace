...
void bailey_fft_complex(Complex *In, int log2_nfft, Complex *Out)
{
    int N = 1 << log2_nfft;
    int log2_r = log2_nfft / 2;
    int log2_c = log2_nfft - log2_r;
    ...
    Complex *stage = (Complex*)malloc(N * sizeof(Complex));
    ...
    // Step 2: FFTs on each column (length nb_row)
    for (int col = 0; col < nb_col; col += NUM_WORKERS)
    {
        int workers = NUM_WORKERS;
        if (workers + col > nb_col){
        ...
        for (int worker = 0; worker < workers; ++worker)
        {
            int base = (col + worker) * nb_row;
            Complex *in_ptr = &stage[base];
            Complex *out_ptr = &temp[base];
            fft(in_ptr, log2_r, out_ptr);
        }
    }
    ...
    for (int col = 0; col < nb_col; col += 1)
    {
        ...
        {
            int index = IDX_COL_MAJOR(row, col, nb_row);
            Complex val = temp[index];

            double angle = -2.0 * M_PI * row * (col) / (double)N;
            Complex twiddle = {(float)cos(angle), (float)sin(angle)};

            // Complex multiplication
            Complex result = {val.real() * twiddle.real() - val.imag() * twiddle.imag(), val.real() * twiddle.imag() + val.imag() * twiddle.real()};
    ...
    for (int row = 0; row < nb_row; ++row)
    {
        for (int col = 0; col < nb_col; ++col)
        {
            int row_index = IDX_ROW_MAJOR(row, col, nb_col);
            int col_index = IDX_COL_MAJOR(row, col, nb_row);
            stage[row_index] = temp[col_index];
        }
    }
    ...
    for (int row = 0; row < nb_row; row += NUM_WORKERS)
    {
        ...
        for (int worker = 0; worker < workers; ++worker)
        {
            int base = (row + worker) * nb_col;
            Complex *in_ptr = &stage[base];
            Complex *out_ptr = &temp[base];
            fft(in_ptr, log2_c, out_ptr);
        }
    }
...
int main(int argc, char **argv) {
    ...
    int log2_nfft = 10; //between 10 and 19
    ...
    float* input_real_fxp = (float*)malloc(sizeInput * sizeof(FXP_TYPE));
    float* input_imag_fxp = (float*)malloc(sizeInput * sizeof(FXP_TYPE));
    ...
    float* outputHW_real_fxp = (float*)malloc(sizeOutput * sizeof(FXP_TYPE));
    float* outputHW_imag_fxp = (float*)malloc(sizeOutput * sizeof(FXP_TYPE));
    float* outputHW_real = (float*)malloc(sizeOutput * sizeof(FFT_TYPE));
    float* outputHW_imag = (float*)malloc(sizeOutput * sizeof(FFT_TYPE));
    ...
    for (uint32_t i = 0; i < nfft; ++i)
    {
        ...
        #endif
        input_real_fxp[i] = input_complex[i].real();
        input_imag_fxp[i] = input_complex[i].imag();
    ...
    bailey_fft_complex(input_complex, log2_nfft, outputSW);

    fft_hw(input_real_fxp, input_imag_fxp, log2_nfft, outputHW_real_fxp, outputHW_imag_fxp);
    for (int i = 0; i < sizeOutput; ++i)
    {
    	outputHW_real[i] = outputHW_real_fxp[i]; // de-normalize the output
    	outputHW_imag[i] = outputHW_imag_fxp[i]; // de-normalize the output
    }
