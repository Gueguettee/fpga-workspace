...
// #define BAILEY_SOFTWARE
#define BAILEY_HARDWARE
// #define ACP_CACHE   // Not compatible with double AXI !!!
// #define DOUBLE_AXI
...
FXP_TYPE reverse_bits_fxp(FXP_TYPE input, int num_stages) {
    FXP_TYPE rev = 0;
...
void bit_reverse_fxp(const FXP_TYPE* X_real, const FXP_TYPE* X_imag, int nfft, int num_stages,
                 FXP_TYPE* OUT_real, FXP_TYPE* OUT_imag) {
    FXP_TYPE reversed;
    for (int i = 0; i < nfft; i++) {
        reversed = reverse_bits_fxp(i, num_stages);
...
void fft_stage_fxp(int stage, FXP_TYPE* Xr, FXP_TYPE* Xi, int nfft,
               FXP_TYPE* Outr, FXP_TYPE* Outi,
               const FXP_TYPE* W_real, const FXP_TYPE* W_imag) {
    int DFTpts = 1 << stage;
    int numBF = DFTpts / 2;
    int twiddle_idx;
    FXP_TYPE wr, wi;
    FXP_TYPE tr, ti;
    int il;
    int base = MAX_NSTAGES_BAILEY - stage;
    int stride = 1 << base;

    for (int j = 0; j < numBF; j++) {
        twiddle_idx = j * stride;
        wr = W_real[twiddle_idx];
        wi = W_imag[twiddle_idx];
        for (int i = j; i < nfft; i += DFTpts) {
            il = i + numBF;

            FXP_Mult_complex(&Xr[il], &Xi[il], &wr, &wi, &tr, &ti);
...
void fft_fxp(FXP_TYPE* In_real, FXP_TYPE* In_imag, int log2_nfft,
         FXP_TYPE* Out_real, FXP_TYPE* Out_imag,
         FXP_TYPE *W_real, FXP_TYPE *W_imag) {
    
    int nstages = log2_nfft;
    int nfft = 1 << nstages;
    FXP_TYPE Stage_real[2][nfft];
    FXP_TYPE Stage_imag[2][nfft];

    bit_reverse_fxp(In_real, In_imag, nfft, nstages, Stage_real[0], Stage_imag[0]);
    ...
    for (int stage = 1; stage < nstages; stage++) {
        ...
        index3 = (index+1)%2;
        fft_stage_fxp(stage,
                  Stage_real[index2], Stage_imag[index2],
                  nfft,
                  Stage_real[index3], Stage_imag[index3],
                  W_real, W_imag);
        index++;
    }

    fft_stage_fxp(nstages,
              Stage_real[index3], Stage_imag[index3],
              nfft,
              Out_real, Out_imag,
              W_real, W_imag);
...
void welch_psd(std::complex<float>* samples, int Nseg, float Fs, float Fc, float* freqs, int log2_nfft, float* spectrum, TTimes & times, CXADCProxy *powerRanger, TEnergies & energies, CFFTProxy *fftProxy){
...
#ifdef BAILEY_HARDWARE
    float* temp_input_real = (float*)fftProxy->AllocDMACompatible(nfft * sizeof(float), 1);
    float* temp_input_imag = (float*)fftProxy->AllocDMACompatible(nfft * sizeof(float), 1);
    float* temp_output_real = (float*)fftProxy->AllocDMACompatible(nfft * sizeof(float), 1);
    float* temp_output_imag = (float*)fftProxy->AllocDMACompatible(nfft * sizeof(float), 1);
#endif

#ifdef ACP_CACHE
    FXP_TYPE *stage_real = (FXP_TYPE*)malloc(nfft * sizeof(FXP_TYPE));
    FXP_TYPE *stage_imag = (FXP_TYPE*)malloc(nfft * sizeof(FXP_TYPE));
    FXP_TYPE *temp_real = (FXP_TYPE*)malloc(nfft * sizeof(FXP_TYPE));
    FXP_TYPE *temp_imag = (FXP_TYPE*)malloc(nfft * sizeof(FXP_TYPE));
#else
#ifdef BAILEY_HARDWARE
    FXP_TYPE *stage_real;
    FXP_TYPE *stage_imag;
    ...
    FXP_TYPE *temp_imag;
#else
    FXP_TYPE *stage_real = (FXP_TYPE*)fftProxy->AllocDMACompatible(nfft * sizeof(FXP_TYPE));
    FXP_TYPE *stage_imag = (FXP_TYPE*)fftProxy->AllocDMACompatible(nfft * sizeof(FXP_TYPE));
    FXP_TYPE *temp_real = (FXP_TYPE*)fftProxy->AllocDMACompatible(nfft * sizeof(FXP_TYPE));
    FXP_TYPE *temp_imag = (FXP_TYPE*)fftProxy->AllocDMACompatible(nfft * sizeof(FXP_TYPE));
#endif
#endif
    ...
    for (int row = 0; row < nb_row; ++row)
    {
        for (int col = 0; col < nb_col; ++col)
        {
            ...
            double angle = -2.0 * M_PI * row * col / nfft;
            twiddle_cos[index] = Float2Fxp((double)cos(angle));
            twiddle_sin[index] = Float2Fxp((double)sin(angle));
        }
    }
#endif

#ifndef BAILEY_SOFTWARE
#ifdef ACP_CACHE
...
#endif
#else
    FXP_TYPE* W_real = (FXP_TYPE*)malloc(MAX_NFFT_BAILEY/2 * sizeof(FXP_TYPE));
    FXP_TYPE* W_imag = (FXP_TYPE*)malloc(MAX_NFFT_BAILEY/2 * sizeof(FXP_TYPE));
    double e = -2.0f * M_PI  / MAX_NFFT_BAILEY;
    #pragma omp parallel for
    for (int j = 0; j < MAX_NFFT_BAILEY/2; ++j){
        double angle = e * j;
        W_real[j] = Float2Fxp((double)cos(angle));
        W_imag[j] = Float2Fxp((double)sin(angle));
    }
#endif
    ...
    for (int i = 0; i < Nseg-3; i+=1){ // Slide 1 block
        ...
        for (i = 0; i < nfft; ++i) {
            temp_input_real[i] = samples_buffer[i].real();
            temp_input_imag[i] = samples_buffer[i].imag();
        }
        fftProxy->FFT_HW(temp_input_real, temp_input_imag, log2_nfft, temp_output_real, temp_output_imag, 0);
        for (i = 0; i < nfft; ++i) {
            coeff[i] = {temp_output_real[i], temp_output_imag[i]};
        }
    ...
    free(coeff);
#ifdef BAILEY_HARDWARE
    fftProxy->FreeDMACompatible(temp_input_real);
    fftProxy->FreeDMACompatible(temp_input_imag);
    fftProxy->FreeDMACompatible(temp_output_real);
    fftProxy->FreeDMACompatible(temp_output_imag);
#endif
#ifdef ACP_CACHE
    free(stage_real);
    free(stage_imag);
...
#else
#ifndef BAILEY_HARDWARE
    fftProxy->FreeDMACompatible(stage_real);
    fftProxy->FreeDMACompatible(stage_imag);
    fftProxy->FreeDMACompatible(temp_real);
    fftProxy->FreeDMACompatible(temp_imag);
#endif
#endif
#ifndef BAILEY_SOFTWARE
#ifdef ACP_CACHE
                ...
                uint64_t &timeStep1, uint64_t &timeStep2, uint64_t &timeStep3, uint64_t &timeStep4, uint64_t &timeStep5)
{
    ...
    */ 
    ...
    for (int col = 0; col < nb_col; ++col)
    {
        for (int row = 0; row < nb_row; ++row)
        {
            ...
            stage_real[col_index] = Float2Fxp(samples_buffer[row_index].real());
            stage_imag[col_index] = Float2Fxp(samples_buffer[row_index].imag());
        }
    }
#ifdef CALCUL_TIME
    clock_gettime(CLOCK_MONOTONIC_RAW, &end_segment);
    ...
    */
...
#ifdef DOUBLE_AXI
    fftProxy->flushCache(stage_real, N * sizeof(FXP_TYPE));
    fftProxy->flushCache(stage_imag, N * sizeof(FXP_TYPE));

    fftProxy->invalidateCache(temp_real, N * sizeof(FXP_TYPE));
    fftProxy->invalidateCache(temp_imag, N * sizeof(FXP_TYPE));
#endif

#ifdef BAILEY_SOFTWARE
    #pragma omp parallel for
    for (int col = 0; col < nb_col; col++) {
        int col_index = col * nb_row;
        fft_fxp(&stage_real[col_index], &stage_imag[col_index], log2_r,
                &temp_real[col_index], &temp_imag[col_index], W_real, W_imag);
    }
#else
    for (int col = 0; col < nb_col; col += NUM_WORKERS)
    {
        ...
        // fftProxy->FFT_HW(cache_in_real, cache_in_imag, log2_r, cache_out_real, cache_out_imag, 0);
        for (int i = 0; i < workers*nb_row; ++i) {
            temp_real[col_index+i] = cache_out_real[i];
            temp_imag[col_index+i] = cache_out_imag[i];
        }
#else
        int col_index = col * nb_row * sizeof(FXP_TYPE);
        // fftProxy->FFT_HW(stage_real, stage_imag, log2_r, temp_real, temp_imag, col_index);
#endif        
    }
#endif
    ...
    /*
    *** Step 3: Multiply by twiddle factor and convert to row major ***
    */ 
    #pragma omp parallel for
    for (int col = 0; col < nb_col; ++col)
    {
        for (int row = 0; row < nb_row; ++row)
        {
			...
			FXP_TYPE wr = twiddle_cos[col_index];
            FXP_TYPE wi = twiddle_sin[col_index];
    ...
    */ 
    ...
    for (int row = 0; row < nb_row; row++) {
        ...
        fft_fxp(&stage_real[row_index], &stage_imag[row_index], log2_c,
                &temp_real[row_index], &temp_imag[row_index], W_real, W_imag);
    }
#else
    for (int row = 0; row < nb_row; row += NUM_WORKERS)
    {
        ...
        // fftProxy->FFT_HW(cache_in_real, cache_in_imag, log2_c, cache_out_real, cache_out_imag, 0);
        for (int i = 0; i < workers*nb_col; ++i) {
        ...
        // fftProxy->FFT_HW(stage_real, stage_imag, log2_c, temp_real, temp_imag, row_index);
#endif       
    }
#endif
    ...
    for (int row = 0; row < nb_row; ++row)
    {
        for (int col = 0; col < nb_col; ++col)
        {
        	int row_index = IDX_ROW_MAJOR(row, col, nb_col);
        	int col_index = IDX_COL_MAJOR(row, col, nb_row);
            coeff[col_index] = std::complex<float>(Fxp2Float(temp_real[row_index]) * N, Fxp2Float(temp_imag[row_index]) * N);
        }
    }
#ifdef CALCUL_TIME
    clock_gettime(CLOCK_MONOTONIC_RAW, &end_segment);
