...
- [Solution 4](https://gitlab.epfl.ch/gjenni/galactic-speed-project/-/tree/main/Versions/V4?ref_type=heads)
...
This solutions is using the Bailey's algorithm, with the FFT computed inside the hardware. This version is using caching (with ACP bus) and parrallelisation inside the hardware (with HLS dataflow). There are a lot of updates compared to the V2, for example this version is using 2 AXI bus (one for the real values and one the imag), connected to the ACP port to use caching.
...
## Solution V4: Bailey's FFT fully software

[Solution link](https://gitlab.epfl.ch/gjenni/galactic-speed-project/-/tree/main/Versions/V4?ref_type=heads)

This solutions is using the Bailey's algorithm, with the FFT computed 100% inside the software. This version is using 2 threads to accelerate the execution in some parts.

This version is **compatible for all the configs (0 to 9).**

Here you'll find the performance, ressources and energy used for this solution:
![Table of V4 solution](./Versions/V4/V4.png)
