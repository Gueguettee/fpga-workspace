...
void CAccelProxy::flushCache(void *VirtAddr, uint32_t LENGTH_IN_BYTES)
{
    cma_flush_cache(VirtAddr, dmaMappings[(uint32_t)VirtAddr], LENGTH_IN_BYTES);
}
...
void CAccelProxy::invalidateCache(void *VirtAddr, uint32_t LENGTH_IN_BYTES)
{
    cma_invalidate_cache(VirtAddr, dmaMappings[(uint32_t)VirtAddr], LENGTH_IN_BYTES);
}
