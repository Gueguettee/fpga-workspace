...
void bit_reverse_fxp(const FXP_TYPE* X_real, const FXP_TYPE* X_imag, int nfft, int num_stages,
    ...
    FXP_TYPE reversed;
    for (int i = 0; i < nfft; i++) {
        reversed = reverse_bits(i, num_stages);
        if (i <= reversed) {
...
void fft_stage_fxp(int stage, FXP_TYPE* Xr, FXP_TYPE* Xi, int nfft,
    ...
    for (int j = 0; j < numBF; j++) {
        int twiddle_idx = j << (MAX_NSTAGES_BAILEY-stage);
        FXP_TYPE wr = W_real[twiddle_idx];
        FXP_TYPE wi = W_imag[twiddle_idx];
        for (int i = j; i < nfft; i += DFTpts) {
            int il = i + numBF;
...
void welch_psd(std::complex<float>* samples, int Nseg, float Fs, float Fc, float* freqs, int log2_nfft, float* spectrum, TTimes & times, CXADCProxy *powerRanger, TEnergies & energies, CFFTProxy *fftProxy){
...
#else
    FXP_TYPE *stage_real = (FXP_TYPE*)fftProxy->AllocDMACompatible(nfft * sizeof(FXP_TYPE), true);
    FXP_TYPE *stage_imag = (FXP_TYPE*)fftProxy->AllocDMACompatible(nfft * sizeof(FXP_TYPE), true);
    FXP_TYPE *temp_real = (FXP_TYPE*)fftProxy->AllocDMACompatible(nfft * sizeof(FXP_TYPE), true);
    FXP_TYPE *temp_imag = (FXP_TYPE*)fftProxy->AllocDMACompatible(nfft * sizeof(FXP_TYPE), true);
#endif
    ...
    float e = -2.0f * M_PI  / MAX_NFFT_BAILEY;
    for (int j = 0; j < MAX_NFFT_BAILEY/2; ++j){
        float angle = e * j;
        W_real[j] = Float2Fxp((float)cos(angle));
        W_imag[j] = Float2Fxp((float)sin(angle));
    }
#endif
    ...
    for (int i = 0; i < Nseg-3; i+=1){ // Slide 1 block
        ...
        bailey_fft(samples_buffer, log2_nfft, coeff, fftProxy, twiddle_cos, twiddle_sin, W_real, W_imag, stage_real, stage_imag, temp_real, temp_imag, nullptr, nullptr, nullptr, nullptr,
        ...
        for (int col = 0; col < nb_col; ++col)
        {
            for (int row = 0; row < nb_row; ++row)
            {
                ...
                stage_real[row_index] = Float2Fxp(samples_buffer[row_index].real());
                stage_imag[row_index] = Float2Fxp(samples_buffer[row_index].imag());
            }
        }

        fftProxy->flushCache(stage_real, nfft * sizeof(FXP_TYPE));
        fftProxy->flushCache(stage_imag, nfft * sizeof(FXP_TYPE));

        fftProxy->invalidateCache(temp_real, nfft * sizeof(FXP_TYPE));
        fftProxy->invalidateCache(temp_imag, nfft * sizeof(FXP_TYPE));

        fftProxy->FFT_HW(stage_real, stage_imag, log2_nfft, temp_real, temp_imag);
        ...
        for (int col = 0; col < nb_col; ++col)
        {
            ...
            {
                int row_index = IDX_ROW_MAJOR(row, col, nb_col);
                coeff[row_index] = std::complex<float>(Fxp2Float(temp_real[row_index]) * nfft, Fxp2Float(temp_imag[row_index]) * nfft);
            }
        }
#else
#ifdef ACP_CACHE
        bailey_fft(samples_buffer, log2_nfft, coeff, fftProxy, twiddle_cos, twiddle_sin, nullptr, nullptr, stage_real, stage_imag, temp_real, temp_imag, cache_in_real, cache_in_imag, cache_out_real, cache_out_imag,
                                timeStep1, timeStep2, timeStep3, timeStep4, timeStep5);
#else
        bailey_fft(samples_buffer, log2_nfft, coeff, fftProxy, twiddle_cos, twiddle_sin, nullptr, nullptr, stage_real, stage_imag, temp_real, temp_imag, nullptr, nullptr, nullptr, nullptr,
                                timeStep1, timeStep2, timeStep3, timeStep4, timeStep5);
#endif
#endif
#endif
                ...
                uint64_t &timeStep1, uint64_t &timeStep2, uint64_t &timeStep3, uint64_t &timeStep4, uint64_t &timeStep5)
{
    ...
    */
    ...
    for (int col = 0; col < nb_col; col += 1)
    {
        ...
        fft_fxp(&stage_real[col_index], &stage_imag[col_index], log2_r, &temp_real[col_index], &temp_imag[col_index], W_real, W_imag);
    }
#else
    for (int col = 0; col < nb_col; col += NUM_WORKERS)
    {
        ...
        fftProxy->FFT_HW(stage_real, stage_imag, log2_r, temp_real, temp_imag, col_index, workers);
#endif        
    }
#endif
    ...
    */ 
    ...
    for (int row = 0; row < nb_row; ++row)
    {
        ...
        fft_fxp(&stage_real[row_index], &stage_imag[row_index], log2_c, &temp_real[row_index], &temp_imag[row_index], W_real, W_imag);
    }
#else
    for (int row = 0; row < nb_row; row += NUM_WORKERS)
    {
        ...
        fftProxy->FFT_HW(stage_real, stage_imag, log2_c, temp_real, temp_imag, row_index, workers);
#endif       
    }
#endif
