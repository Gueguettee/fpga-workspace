...
int main(int argc, char ** argv)
{
  ...
  printf("inputImageFxp: Virtual address: 0x%08X (%u)\n", (uint32_t)inputImageFxp, (uint32_t)inputImageFxp);
  if (inputImageFxp == NULL) {
    printf("Error allocating %" PRIu32 " bytes for the input image in FxP\n", INPUT_SIZE * sizeof(TFXP));
    return false;
  }

  if (!LoadImageInFxp(argv[1], inputImageFxp, inputImage, INPUT_SIZE)) {
    printf("Error loading the image file.\n");
    FreeParamsDMA(convolver, NUM_LAYERS, (void**)weights);
    FreeParamsDMA(convolver, NUM_LAYERS, (void**)biases);
    return -1;
  }
