...
TFXP tempInput[12192];
//#pragma HLS BIND_STORAGE variable=tempInput type=ram_2p impl=lutram
  ...
  #pragma HLS LOOP_TRIPCOUNT min=MAX_FILTERS max=MAX_FILTERS
    ...
    #pragma HLS LOOP_TRIPCOUNT min=MAX_CHANNELS max=MAX_CHANNELS
      ...
      #pragma HLS LOOP_TRIPCOUNT min=3 max=3
        ...
        #pragma HLS LOOP_TRIPCOUNT min=MAX_WIDTH max=MAX_WIDTH
          tempInput[iChannel*3*inputWidth + cy*inputWidth + x] = *(input + iChannel*inputHeight*inputWidth + cy*inputWidth + x);
        }
      }
    }
    TFXP iCy = 0;
    ...
    #pragma HLS LOOP_TRIPCOUNT min=MAX_HEIGHT-2 max=MAX_HEIGHT-2
        ...
        #pragma HLS LOOP_TRIPCOUNT min=MAX_CHANNELS max=MAX_CHANNELS
          ...
          #pragma HLS LOOP_TRIPCOUNT min=3 max=3
            ...
            #pragma HLS LOOP_TRIPCOUNT min=3 max=3
              ...
              v = tempInput[iChannel*3*inputWidth + iiCy*inputWidth+ x+cx];
      ...
      #pragma HLS LOOP_TRIPCOUNT min=MAX_CHANNELS max=MAX_CHANNELS
        for (TFXP x = 0; x < inputWidth; ++x) {
        #pragma HLS LOOP_TRIPCOUNT min=MAX_WIDTH max=MAX_WIDTH
          tempInput[iChannel*3*inputWidth + iCy*inputWidth+ x] = *(input + iChannel*inputHeight*inputWidth + (y+3)*inputWidth + x);
        }
      }
      iCy += 1;
      if (iCy == 3) {
