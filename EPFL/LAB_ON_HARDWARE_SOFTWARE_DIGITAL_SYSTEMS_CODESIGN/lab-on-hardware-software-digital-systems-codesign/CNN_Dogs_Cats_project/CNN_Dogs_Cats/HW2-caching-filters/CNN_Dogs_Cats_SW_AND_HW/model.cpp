...
TFXP Inference(CConv2DDriver &convolver, TFXP * inputImageFxp, TFXP * buffer0, TFXP * buffer1, TFXP ** fxpWeights, TFXP ** fxpBiases, TTimes & times)
{
  ...
  convolver.Conv2D_HW(inputImageFxp, buffer0, fxpWeights[iLayer], LayerShapes[iLayer][1], LayerShapes[iLayer][0], size, size, 3, 3);
  size -= 2;
  ...
  size = 127;
  clock_gettime(CLOCK_MONOTONIC_RAW, &start);
  convolver.Conv2D_HW(buffer1, buffer0, fxpWeights[iLayer], LayerShapes[iLayer][1], LayerShapes[iLayer][0], size, size, 3, 3);
  size -= 2;
  AddBiases(buffer0, fxpBiases[iLayer], LayerShapes[iLayer][1], size, size);
  ReLU(buffer0, LayerShapes[iLayer][1], size, size);
  clock_gettime(CLOCK_MONOTONIC_RAW, &end);
