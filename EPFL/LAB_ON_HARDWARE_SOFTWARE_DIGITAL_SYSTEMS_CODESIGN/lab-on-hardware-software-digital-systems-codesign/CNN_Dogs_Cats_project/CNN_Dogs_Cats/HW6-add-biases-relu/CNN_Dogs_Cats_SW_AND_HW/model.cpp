...
bool ConvertWeightsToFxP(CConv2DDriver &convolver, const uint32_t numLayers, float ** floatWeights, TFXP ** fxpWeights)
{
  float * pFloat;
  TFXP * pFxp;
...
void FreeParamsDMA(CConv2DDriver &convolver, const uint32_t numLayers, void ** params)
{
  for (uint32_t ii = 0; ii < numLayers; ++ ii) {
    if (params[ii]) {
...
bool ConvertBiasesToFxP(CConv2DDriver &convolver, const uint32_t numLayers, float ** floatBiases, TFXP ** fxpBiases)
{
  float * pFloat;
  TFXP * pFxp;
...
bool LoadModelInFxP(CConv2DDriver &convolver, TFXP ** fxpWeights, TFXP ** fxpBiases)
{
  ...
  if (!LoadFloatBiases(NUM_LAYERS, floatBiases)) {
    ...
    FreeParamsDMA(convolver, NUM_LAYERS, (void**)fxpWeights);
    return false;
  }
...
TFXP Inference(CConv2DDriver &convolver, TFXP * inputImageFxp, TFXP * buffer0, TFXP * buffer1, TFXP ** fxpWeights, TFXP ** fxpBiases, TTimes & times)
{
  uint32_t iLayer, size;
  struct timespec start, end;

  buffer0 = (TFXP *)convolver.AllocDMACompatible(4129024 * sizeof(TFXP));
  memset(buffer0, 0, 4129024 * sizeof(TFXP));
  buffer1 = (TFXP *)convolver.AllocDMACompatible(1032256 * sizeof(TFXP));
  memset(buffer1, 0, 1032256 * sizeof(TFXP));
  
  iLayer = 0, size = 256;
  clock_gettime(CLOCK_MONOTONIC_RAW, &start);
  convolver.Conv2D_HW(inputImageFxp, buffer0, fxpWeights[iLayer], fxpBiases[iLayer], LayerShapes[iLayer][1], LayerShapes[iLayer][0], size, size, 3, 3);
  size -= 2;
  ...
  printf("First convoluation done\n");

  size = 127;
  clock_gettime(CLOCK_MONOTONIC_RAW, &start);
  convolver.Conv2D_HW(buffer1, buffer0, fxpWeights[iLayer], fxpBiases[iLayer], LayerShapes[iLayer][1], LayerShapes[iLayer][0], size, size, 3, 3);
  size -= 2;
  // AddBiases(buffer0, fxpBiases[iLayer], LayerShapes[iLayer][1], size, size);
  // ReLU(buffer0, LayerShapes[iLayer][1], size, size);
  clock_gettime(CLOCK_MONOTONIC_RAW, &end);
  ...
  printf("Final predication before output: %u\n", buffer0[0]);
  TFXP finalPrediction = buffer0[0];
  convolver.FreeDMACompatible(buffer0);
  convolver.FreeDMACompatible(buffer1);
  convolver.FreeDMACompatible(inputImageFxp);
  for (uint32_t ii = 0; ii < NUM_LAYERS; ++ ii) {
    convolver.FreeDMACompatible(fxpWeights[ii]);
    convolver.FreeDMACompatible(fxpBiases[ii]);
  }
