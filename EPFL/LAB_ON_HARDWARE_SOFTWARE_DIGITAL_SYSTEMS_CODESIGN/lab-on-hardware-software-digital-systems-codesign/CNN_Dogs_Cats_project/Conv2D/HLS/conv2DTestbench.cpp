      ...
      uint32_t convWidth = 3, uint32_t convHeight = 3)
{
  ...
  for (uint32_t ii = 0; ii < numFilters; ++ ii) {
    for (uint32_t jj = 0; jj < (inputWidth-2)*(inputHeight-2); ++ jj) {
      output[ii * (inputWidth-2)*(inputHeight-2) + jj] += biases[ii];
      if (Fxp2Float(output[ii * (inputWidth-2)*(inputHeight-2) + jj], DECIMALS) < 0.0)
        output[ii * (inputWidth-2)*(inputHeight-2) + jj] = 0;
    }
  }
}

///////////////////////////////////////////////////////////////////////////////
void InitVectors(TFXP * input, uint32_t sizeInput, TFXP * coeffs, uint32_t sizeCoeffs, TFXP * biases, uint32_t sizeBiases)
{
...
int main(int argc, char ** argv)
{
  ...
  for (uint32_t iTest = 0; iTest < numSizes; ++ iTest) {
    ...
    printf("  SW\n");
    Conv2D_SW(input, outputSW, coeffs, biases, channels, filters, width_height[iTest][0], width_height[iTest][1], 3, 3);
    printf("  HW\n");
    Conv2D_HW(input, outputHW, coeffs, biases, channels, filters, width_height[iTest][0], width_height[iTest][1], 3, 3);
