...
void load_next_input(TFXP *input, TFXP buffer[4064], TFXP inputWidth, TFXP inputHeight, TFXP numChannels, TFXP y) {
  ...
  #pragma HLS LOOP_TRIPCOUNT min=MAX_CHANNELS max=MAX_CHANNELS
    ...
    #pragma HLS LOOP_TRIPCOUNT min=MAX_WIDTH max=MAX_WIDTH
      buffer[iChannel*inputWidth + x] = *(input + iChannel*inputHeight*inputWidth + y*inputWidth + x);
    }
  }
}
...
#pragma HLS INTERFACE m_axi depth=516128 port=input offset=slave //MAX_WIDTH * MAX_HEIGHT * MAX_CHANNELS
#pragma HLS INTERFACE m_axi depth=2097152 port=output offset=slave //num_write_outstanding=N_UNROLL_OUTPUT //MAX_WIDTH * MAX_HEIGHT * MAX_FILTERS
#pragma HLS INTERFACE m_axi depth=294912 port=filters offset=slave
#pragma HLS INTERFACE m_axi depth=MAX_FILTERS port=biases offset=slave
  ...
  #pragma HLS array_partition variable=tempFilters type=cyclic factor=9*N_UNROLL_OUTPUT dim=2
	...
	TFXP tempInput[4][4064];
  #pragma HLS bind_storage variable=tempInput type=ram_s2p
  #pragma HLS array_partition variable=tempInput type=cyclic factor=4 dim=1
  #pragma HLS array_partition variable=tempInput type=cyclic factor=3 dim=2

  TFXP acc[N_UNROLL_OUTPUT];
  #pragma HLS array_partition variable=acc type=cyclic factor=N_UNROLL_OUTPUT dim=0

  TFXP tempBiase[N_UNROLL_OUTPUT];
  #pragma HLS array_partition variable=tempBiase type=cyclic factor=N_UNROLL_OUTPUT dim=0
  ...
  #pragma HLS LOOP_TRIPCOUNT min=MAX_FILTERS max=MAX_FILTERS
    ...
    #pragma HLS unroll factor=N_UNROLL_OUTPUT
      tempBiase[iF] = *(biases + iFilter + iF);
    }
    for (TFXP iChannel = 0; iChannel < numChannels; ++ iChannel) {
    #pragma HLS LOOP_TRIPCOUNT min=MAX_CHANNELS max=MAX_CHANNELS
    ...
    #pragma HLS LOOP_TRIPCOUNT min=MAX_HEIGHT-2 max=MAX_HEIGHT-2
	  #pragma HLS DATAFLOW
      if (y+3 < inputHeight) {
        load_next_input(input, tempInput[(iCy+3)%4], inputWidth, inputHeight, numChannels, y+3);
      }
      ...
      #pragma HLS LOOP_TRIPCOUNT min=MAX_WIDTH-2 max=MAX_WIDTH-2
        ...
        #pragma HLS LOOP_TRIPCOUNT min=MAX_CHANNELS max=MAX_CHANNELS
          ...
          #pragma HLS unroll factor=9*N_UNROLL_OUTPUT
            TFXP cxy = cxyf % 9;
            TFXP iF = cxyf / 9;
            TFXP v = tempInput[(cxy/3+iCy) % 4][iChannel * inputWidth + x + cxy % 3];
