...
const uint32_t SEQMATCHER_HW_ADDR = 0x40000000; // From Vivado's address editor


///////////////////////////////////////////////////////////////////////////////
bool InitDevice(CSeqMatcherDriver & seqMatcher, uint32_t numDBEntries, uint32_t numSeqsSpecimen,
    char * &seqsDB, char * &seqsSpecimen,
    uint8_t * &lengthsDB, uint8_t * &lengthsSpecimen, int8_t * &scores, bool log=true)
{
  ...
  if ( seqMatcher.Open(SEQMATCHER_HW_ADDR, MAP_SIZE) != CAccelDriver::OK ) {
  ...
  seqsDB = (char *)seqMatcher.AllocDMACompatible(numDBEntries*MAX_SEQ_LENGTH*sizeof(char));
  lengthsDB = (uint8_t *)seqMatcher.AllocDMACompatible(numDBEntries*sizeof(uint8_t));
  seqsSpecimen = (char *)seqMatcher.AllocDMACompatible(numSeqsSpecimen*MAX_SEQ_LENGTH*sizeof(char));
...
int main(int argc, char ** argv)
{
  ...
  char * seqsDB, * seqsSpecimen; // Have to be allocated for DMA access
  ...
  CSeqMatcherDriver seqMatcher(false); // Deactivat logging
