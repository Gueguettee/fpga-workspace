...
uint32_t CAccelProxy::Open(uint64_t BaseAddr, uint32_t MappingSize, volatile void ** AccelRegsPointer)
{
  if (logging)
    printf("CAccelProxy::Open(BaseAddr = 0x%016lX, Size = %u)\n", BaseAddr, MappingSize);
...
{
  if (logging)
    printf("CAccelProxy::FreeDMACompatible(Addr = 0x%016lX)\n", (uint64_t)VirtAddr);
...
uint64_t CAccelProxy::GetDMAPhysicalAddr(void * VirtAddr)
{
  if (logging)
    printf("CAccelProxy::GetDMAPhysicalAddr(Addr = 0x%016lX)\n", (uint64_t)VirtAddr);
...
{
  uint64_t numMappings = dmaMappings.size();

  if (logging)
    printf("CAccelProxy::InternalEmptyDMAAllocs(DMA dict size = %lu)\n", numMappings);
